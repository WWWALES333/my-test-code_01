...
class Tagger:
    ...
    def classify(self, text: str, context: Dict[str, object] | None = None) -> Dict[str, object]:
        base = self._classify_mock(text, context or {})
        if self.mode == "mock":
            base["model_mode"] = "mock"
            base["model_name"] = "mock-rule-engine"
            return base

        try:
            refined = self._classify_real(text, context or {}, base)
            ...
            "messages": [
                {
                    ...
                    "content": (
                        "你是销售周报AI专题分类器。"
                        "请仅返回JSON对象，不要包含其他文本。"
                        "字段: is_ai_hit,business_line,actor_primary,actor_subtype,ai_scope,"
                        "interaction_outcome,certainty_level,review_reason_code,decision_status,confidence,reason。"
                    ),
                },
                {
                    "role": "user",
                    "content": json.dumps(
                        {
                            ...
                            "constraints": {
                                ...
                                "review_reason_code_values": sorted(list(REVIEW_REASON_CODE_VALUES)),
...
def _normalize_model_result(raw: Dict[str, object]) -> Dict[str, object]:
    ...
    actor_subtype_raw = raw.get("actor_subtype", "")
    if isinstance(actor_subtype_raw, list):
        actor_subtype = ";".join([str(x).strip() for x in actor_subtype_raw if str(x).strip()][:2])
    else:
        actor_subtype = str(actor_subtype_raw).strip()

    review_reason_code_raw = raw.get("review_reason_code", "")
    if isinstance(review_reason_code_raw, list):
        codes = [str(x).strip() for x in review_reason_code_raw if str(x).strip()]
        review_reason_code = ";".join(codes)
    else:
        review_reason_code = str(review_reason_code_raw).strip()

    decision_status = str(raw.get("decision_status", "")).strip()
    ...
    interaction_outcome = str(raw.get("interaction_outcome", "")).strip()
    certainty_level = str(raw.get("certainty_level", "")).strip() or "low"
