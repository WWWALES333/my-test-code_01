...
class WeeklyReportDownloader:
    ...
    def _looks_like_template(self, filename: str) -> bool:
        """判断是否为模板或占位文件"""
        # “战区月报模板2026.3.docx”这类文件虽然含“模板”，但实际是云管家战区月报。
        # 只要文件名同时具备报告类型和明确周期，就应纳入归档统计。
        has_report_period = (
            ("月报" in filename and re.search(r'(20\d{2}|\d{2}年|20\d{2}[.\-]\d{1,2}|\d{1,2}月|[一二三四五六七八九十]+月)', filename))
            or ("周报" in filename and re.search(r'(20\d{2}|\d{2}年|\d{1,2}月第?\s*\d{1,2}周|\d{1,2}周)', filename))
        )
    ...
    def generate_audit_report(self, output_dir: Optional[str] = None) -> Dict[str, object]:
        ...
        self._fill_expected_monthly_zero_counts(monthly_counts, weekly_counts)
        monthly_summary = []
    ...
    def _fill_expected_monthly_zero_counts(self, monthly_counts: Dict[str, int], weekly_counts: Dict[str, int]) -> None:
        """补齐已到期但完全缺失的月报月份，避免 0 份月报在审计摘要里消失。"""
        latest_week_month_by_year: Dict[int, int] = {}
        for key in weekly_counts.keys():
            match = re.match(r'(\d{4})/(\d{4})年(\d{2})月第\d+周', key)
            if not match:
                continue
            year = int(match.group(1))
            month = int(match.group(3))
            latest_week_month_by_year[year] = max(latest_week_month_by_year.get(year, 0), month)

        now = datetime.now()
        for year, latest_week_month in latest_week_month_by_year.items():
            if year < now.year:
                expected_end_month = 12
            elif year == now.year:
                # 当前月月报通常尚未完整到齐，只要求到最新周报月份的上一个月。
                expected_end_month = min(max(latest_week_month - 1, 0), max(now.month - 1, 0))
            else:
                expected_end_month = max(latest_week_month - 1, 0)

            for month in range(1, expected_end_month + 1):
                monthly_counts.setdefault(f"{year}/{month:02d}月报", 0)
